...
class GameManager {
    ...
    public void notifyObservers() { for(GameObserver o : observers) o.onGameUpdate(); }

    public void addMoney(int amount) { this.money += amount; notifyObservers(); }
    public void spendMoney(int amount) { this.money -= amount; notifyObservers(); }
    public void takeDamage() { this.lives--; notifyObservers(); if(lives <= 0) state = GameState.GAME_OVER; }

    private void initPath() {
        pathPoints = new Point[] {
                new Point(0, 2 * TILE_SIZE + TILE_SIZE/2), new Point(5 * TILE_SIZE + TILE_SIZE/2, 2 * TILE_SIZE + TILE_SIZE/2),
                new Point(5 * TILE_SIZE + TILE_SIZE/2, 8 * TILE_SIZE + TILE_SIZE/2), new Point(14 * TILE_SIZE + TILE_SIZE/2, 8 * TILE_SIZE + TILE_SIZE/2),
                new Point(14 * TILE_SIZE + TILE_SIZE/2, 4 * TILE_SIZE + TILE_SIZE/2), new Point(MAP_WIDTH, 4 * TILE_SIZE + TILE_SIZE/2)
        };
    }
